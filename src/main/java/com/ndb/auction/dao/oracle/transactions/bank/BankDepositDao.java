...
public class BankDepositDao extends BaseOracleDao {
    ...
    public List<? extends Transaction> selectByUser(int userId, String orderBy, int status) {
        String sql = "SELECT TBL_BANK_DEPOSIT.*, TBL_USER.EMAIL from TBL_BANK_DEPOSIT left JOIN TBL_USER on TBL_BANK_DEPOSIT.USER_ID = TBL_USER.ID WHERE TBL_BANK_DEPOSIT.USER_ID = ? AND TBL_BANK_DEPOSIT.IS_SHOW = ?";
		if (orderBy == null)
			orderBy = "TBL_BANK_DEPOSIT.ID";
		sql += " ORDER BY " + orderBy + " DESC";
    ...
    public Transaction selectById(int id, int status) {
        String sql = "SELECT TBL_BANK_DEPOSIT.*, TBL_USER.EMAIL from TBL_BANK_DEPOSIT left JOIN TBL_USER on TBL_BANK_DEPOSIT.USER_ID = TBL_USER.ID WHERE TBL_BANK_DEPOSIT.ID=? AND TBL_BANK_DEPOSIT.IS_SHOW = ?";
