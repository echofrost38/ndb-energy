...
public class DepositPaypal extends BaseResolver implements GraphQLMutationResolver, GraphQLQueryResolver {
    ...
    @PreAuthorize("isAuthenticated()")
    @SuppressWarnings("unchecked")
    public List<PaypalDepositTransaction> getPaypalDepositTxnsByUser(String orderBy, int status) {
        UserDetailsImpl userDetails = (UserDetailsImpl)SecurityContextHolder.getContext().getAuthentication().getPrincipal();
        int userId = userDetails.getId();
        return (List<PaypalDepositTransaction>) paypalDepositService.selectByUser(userId, orderBy, status);
    }

    @PreAuthorize("isAuthenticated()")
    public PaypalDepositTransaction getPaypalDepositTxnById(int id, int status) {
        UserDetailsImpl userDetails = (UserDetailsImpl)SecurityContextHolder.getContext().getAuthentication().getPrincipal();
        int userId = userDetails.getId();
        var tx = (PaypalDepositTransaction) paypalDepositService.selectById(id, status);
    ...
    @PreAuthorize("isAuthenticated()")
    public int changePayPalDepositShowStatus(int id, int status) {
        return paypalDepositService.changeShowStatus(id, status);
    }
