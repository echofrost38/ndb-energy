...
public class PreSaleOrderDao extends BaseOracleDao {
    ...
    public List<PreSaleOrder> selectByPresaleId(int presaleId) {
        String sql = "SELECT TBL_PRESALE_ORDER.*, TBL_USER_AVATAR.PREFIX, TBL_USER_AVATAR.NAME FROM TBL_PRESALE_ORDER LEFT JOIN TBL_USER_AVATAR ON TBL_PRESALE_ORDER.USER_ID=TBL_USER_AVATAR.ID WHERE TBL_PRESALE_ORDER.PRESALE_ID = ? ORDER BY TBL_PRESALE_ORDER.NDB_AMOUNT DESC";
